...
var moudleDir = path.join(config.rootDirectory, "detailReport", "cardio");
...
function setHandlebars() {
    ...
    handlebars.registerHelper("getImageUrl", function (rootData, filePrefix, fileSuffix, options) {
        var taskUrl = "src/img/down/" + filePrefix + fileSuffix;
        try {
...
function generateDetaileReportTemp(taskName, taskId) {
    ...
    data.cpTargetDirectory = path.join(data.resourcesDirectory, "htmlTempl");
